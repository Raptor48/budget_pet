...
Список расходов за месяц. Поддерживает параметр `query` для поиска.

**Поиск (`query`):** поддерживает диапазоны сумм (`100..300`), операторы (`>100`, `<=50`), даты (`2026-04`), подстроку категории.

**Ответ:**
```json
[
  { "id": 1, "category": "Еда", "amount": 250.0, "date": "2026-04-15" }
]
```
...
Сводка: доходы, долги, минимальные платежи, прогноз закрытия займов.
...
Список доходов за месяц. Опциональный параметр `person`.
